	...
	protected void onResume() {
		...
		if ((-1 == mLat || -1 == mLon) && !isServiceRunning(LocationService.class)) {
			startService(new Intent(getApplicationContext(), LocationService.class)
					.setAction(LocationService.ACTION_LOCATE_ME));
		}
	}
	...
	private class LocationReceiver extends BroadcastReceiver {
		...
		public void onReceive(Context context, Intent intent) {
			...
			if (LocationService.ACTION_LOCATE_ME.equals(action)) {
				Log.d(TAG,
						String.format("Got location %d, %d", intent.getIntExtra(LocationService.LAT, -1),
								intent.getIntExtra(LocationService.LON, -1)));
			}
		}
	}
